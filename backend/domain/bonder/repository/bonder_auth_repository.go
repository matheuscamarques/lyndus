...
type BonderAuthRepository struct {
	conn *sqlx.DB
}

//todo verificar aqui quando implementar authenticação de verdade
func NewBonderAuthRepository() BonderAuthRepository {
	return BonderAuthRepository{
		conn: postgres.DB,
	}
}

//GetUserByCredential Get access by username
func (bar BonderAuthRepository) GetUserByCredential(cnpj types.CNPJ, username string) (a auth.Authentication, err error) {
...
func (bar BonderAuthRepository) GetUserEmailBYUsername(cnpj types.CNPJ, username string) (a auth.Authentication, err error) {
	command := `SELECT aut.id,
       				   bsu.name as username,
       				   bsu.email
            FROM bs_user bsu
...
func (bar BonderAuthRepository) GetEmailBYID(id int) (a auth.Authentication, err error) {
...
func (bar BonderAuthRepository) GetTokenRecovery(id int) (a auth.Authentication, err error) {
	command := `SELECT bsu.authentication_id as id,
						pre.token
...
func (bar BonderAuthRepository) GetUserPasswordByID(id int) (a auth.Authentication, err error) {
	command := `SELECT aut.id,
       				   aut.password
				FROM authentication aut
...
func (bar BonderAuthRepository) CheckPermission(authID, permissionID, levelID int) (bsID int, err error) {
